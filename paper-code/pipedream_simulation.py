...
def run_pipedream_simulation(inp, t_run=24, dt=3600, banded=False, num_iter=40, use_tank_init_cond=False):
    ...
    is_tank = model.superjunctions['tank'].values          #bool, true or false
    ...
    model.spinup(n_steps=10, dt=60, Q_in=Q_in_t, H_bc=H_bc, u_o=u_o, u_p=u_p, banded=banded)
    t3_5=time.time()
    ...
    while model.t < (t_run * 3600):    
        ...
        #%% change the criteria, based on fake node instead of water tank, yifan add
        u_o = ((model.H_j[is_tank_fake_node] > tank_min[is_tank_fake_node]+0.1) & (model.H_j[is_tank_fake_node] < tank_max[is_tank_fake_node]-0.1)).astype(np.float64)
        #%%
...
#        # open link --> 1, close link --> 0
    ...
    return H_df, Q_df, Q_pump, model, Q_in_all_df, pd_time_tot
