...
def wntr_2_pd(wn,t_run, dt):
    ...
    superjunctions_m = pd.DataFrame(columns = ['name','id','z_inv','h_0','bc','storage','a','b','c',
                                               'max_depth','map_x','map_y','dem','tank', 'tank_fake_node','pat', 'tank_min', 'tank_max'], dtype=object)
...
#                ele[i] = min(results.node['head'].loc[:,'FP9001']) # 331.9272
    ...
    superjunctions_m['tank_fake_node'] = tank_fake_node
    ...
    if has_tanks==1:
        ...
        for j in range(num_tanks):
            superjunctions_new=pd.DataFrame(columns=['name','id','z_inv','h_0','bc','storage','a','b','c',
                                                     'max_depth','map_x','map_y','dem','tank','tank_fake_node','pat', 'tank_min', 'tank_max'],
            ...
            superjunctions_new['z_inv']=superjunctions_m['z_inv'][superjunctions_m['tank']== True].values[0]
            ...
            superjunctions_new['tank_fake_node']=True
            superjunctions_new['dem']=0
            superjunctions_new['pat']='-'
            superjunctions_new['tank_min']=superjunctions_m['tank_min'][tank_ind]
    ...
    for i in range(len(pats)):
        ...
        if len(mult_orig)>=t_run:
            mult_loop=mult_orig
        mult_loop=mult_loop.reshape(-1,1)
        if i==0:
            mult=mult_loop
        if i!=0:
    ...
    # just for reservoirs
#    res_pats = ['ANDERSON_MILL_B_MXD20', 'FOUR_POINTS_GST_MXD20']
#    
#    for i in range(len(res_pats)):
#        
#        pat=wn.get_pattern(res_pats[i])
#        mult_orig=pat.multipliers
#        
#        mult_loop=[]
#        if len(mult_orig)<t_run:
...
#            mult=np.hstack((mult,mult_loop))
#    
#    mult_df_res = pd.DataFrame(mult,columns=res_pats)
#    
    ...
    return superjunctions, superlinks, orifices, pumps, H_bc, Q_in, pats, mult_df, tank_min, tank_max, tank_dict, time_controls_compiled, events_controls_pairs
